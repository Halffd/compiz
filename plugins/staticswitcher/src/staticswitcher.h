...
public:
  ...
  bool getPaintRectangle(CompWindow *w, CompRect &rect, int *opacity);
  void doWindowDamage(CompWindow *w);
  void handleSelectionChange(bool toNext, int nextIdx);
  void createPopup();
  bool showPopup();
  Cursor getCursor(bool mouseSelectOn);
  void initiate(SwitchWindowSelection selection, bool shouldShowPopup);
  void windowRemove(CompWindow *w);
  int getRowXOffset(int y);
  void getWindowPosition(unsigned int index, int *x, int *y);
  CompWindow *findWindowAt(int x, int y);
  void handleEvent(XEvent *event);
  bool adjustVelocity();
  void paintRect(const GLMatrix &transform, CompRect &box, int offset,
                 unsigned short *color, unsigned short opacity);
  void paintSelectionRect(const GLMatrix &transform, int x, int y, float dx,
                          float dy, unsigned int opacity);
  void getMinimizedAndMatch(bool &minimizedOption, CompMatch *&match);
  bool getMipmap();
  ...
  bool closeSelected(CompAction *action, CompAction::State state,
  ...
  bool specificZoomN(int level);
  float getZoomLevel();
  bool resetZoom();
  bool setZoomMode(int mode);
  bool setZoomIncrement(float increment);

  // Zoom DBus methods
  bool zoomInDbus(const char *path);
  bool zoomOutDbus(const char *path);
  bool setZoomDbus(const char *path, float level);
  bool specificZoomNDbus(const char *path, int level);
  bool getZoomLevelDbus(const char *path);
  bool resetZoomDbus(const char *path);
  bool setZoomModeDbus(const char *path, int mode);
  bool setZoomIncrementDbus(const char *path, float increment);

  // Quick switch method
  bool quickSwitch(CompAction *action, CompAction::State state,
                   CompOption::Vector &options);
...
public:
  ...
  void paintThumb(const GLWindowPaintAttrib &, const GLMatrix &, unsigned int,
                  int, int);
  void updateIconTexturedWindow(GLWindowPaintAttrib &sAttrib, int &wx, int &wy,
                                int x, int y, GLTexture *icon);
  void updateIconNontexturedWindow(GLWindowPaintAttrib &sAttrib, int &wx,
                                   int &wy, float &width, float &height, int x,
                                   int y, GLTexture *icon);
  void updateIconPos(int &wx, int &wy, int x, int y, float width, float height);

  IconMode getIconMode();

  void drawCloseButton(const GLMatrix &transform, int x, int y,
...
#define SWITCH_SCREEN(s) StaticSwitchScreen *ss = StaticSwitchScreen::get(s)

#define SWITCH_WINDOW(w) StaticSwitchWindow *sw = StaticSwitchWindow::get(w)

class StaticSwitchPluginVTable
    : public CompPlugin::VTableForScreenAndWindow<StaticSwitchScreen,
                                                  StaticSwitchWindow> {
public:
  bool init();
};
