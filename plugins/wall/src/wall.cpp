...
#define sigmoidProgress(x)                                                     \
  ((sigmoid(x) - sigmoid(0)) / (sigmoid(1) - sigmoid(0)))

COMPIZ_PLUGIN_20090315(wall, WallPluginVTable);

WallCairoContext::WallCairoContext()
    : pixmap(0), texture(0), surface(NULL), cr(NULL), width(0), height(0) {}

void WallScreen::clearCairoLayer(cairo_t *cr) {
  cairo_save(cr);
...
void WallScreen::drawSwitcherBackground() {
  ...
  if (radius) {
    ...
    cairo_arc(cr, radius + width - 2 * radius, radius, radius, 1.5f * PI,
              2.0 * PI);
...
void WallScreen::setupCairoContext(WallCairoContext &context) {
  ...
  context.pixmap =
      XCreatePixmap(screen->dpy(), screen->root(), width, height, 32);

  context.texture =
      GLTexture::bindPixmapToTexture(context.pixmap, width, height, 32);
  if (context.texture.empty()) {
    screen->logMessage("wall", CompLogLevelError,
                       "Couldn't create cairo context for switcher");
  }

  context.surface = cairo_xlib_surface_create_with_xrender_format(
      screen->dpy(), context.pixmap, xScreen, format, width, height);

  context.cr = cairo_create(context.surface);
  clearCairoLayer(context.cr);
}

void WallScreen::destroyCairoContext(WallCairoContext &context) {
  if (context.cr)
...
bool WallScreen::checkDestination(unsigned int destX, unsigned int destY) {
  CompPoint point;
  CompSize size;
...
void WallScreen::releaseMoveWindow() { moveWindow = 0; }

void WallScreen::computeTranslation(float &x, float &y) {
...
void WallScreen::determineMovementAngle() {
  int angle;
  float dx, dy;
  ...
  if (dy > IGNORE_REMAINDER)
    angle = (dx > IGNORE_REMAINDER)    ? 135
            : (dx < -IGNORE_REMAINDER) ? 225
                                       : 180;
  else if (dy < -IGNORE_REMAINDER)
    angle = (dx > IGNORE_REMAINDER) ? 45 : (dx < -IGNORE_REMAINDER) ? 315 : 0;
  else
    angle = (dx > IGNORE_REMAINDER) ? 90 : (dx < -IGNORE_REMAINDER) ? 270 : -1;

  direction = angle;
}

bool WallScreen::moveViewport(int x, int y, Window moveWin) {
  ...
  /* For per-monitor workspaces, restrict viewport switching to current monitor
   */
  if (optionGetMmmode() == MmmodePerMonitorWorkspaces &&
      screen->outputDevs().size() > 1) {
    /* In per-monitor mode, we only allow viewport switching on the current
     * monitor */
    int currentOutput = screen->outputDeviceForPoint(mouse.x(), mouse.y());

    /* Calculate which monitor's viewport grid we're working with */
    CompOutput::vector &outputs = screen->outputDevs();
    if (currentOutput >= 0 && currentOutput < (int)outputs.size()) {
      /* For now, implement basic per-monitor switching by limiting
       * the effective viewport size to per-monitor dimensions */
      /* TODO: This could be enhanced to have separate viewport grids per
       * monitor */
    }
  }
  /* For primary monitor only mode, only allow switching if pointer is on
     primary monitor */
  else if (optionGetMmmode() == MmmodePrimaryMonitorOnly &&
           screen->outputDevs().size() > 1) {
    int primaryOutputId = getPrimaryOutputDevice();
    int currentOutput = screen->outputDeviceForPoint(mouse.x(), mouse.y());

    if (currentOutput != primaryOutputId) {
      /* Don't allow viewport switching if not on primary monitor */
      return false;
    }
  }
  ...
  moving = true;
  focusDefault = true;
  boxOutputDevice = screen->outputDeviceForPoint(mouse.x(), mouse.y());
...
void WallScreen::handleEvent(XEvent *event) {
  switch (event->type) {
  case ClientMessage:
 ...
 */
void WallWindow::windowNotify(CompWindowNotify n) {
...
void WallWindow::activate() {
  ...
  if (window->placed() && !screen->otherGrabExist("wall", "switcher", 0)) {
    ...
    if (ws->optionGetAutoSwitchVpAndWindow() ||
        window->defaultViewport() != viewport)
    ...
    if (dx || dy) {
      ...
      if (ws->optionGetAutoSwitchVpAndWindow()) {
        ...
        CompPoint d = compiz::wall::movementWindowOnScreen(
            window->serverBorderRect(), screenRegion);
...
void WallWindow::grabNotify(int x, int y, unsigned int state,
                            unsigned int mask) {
...
void WallWindow::ungrabNotify() {
  WallScreen::get(screen)->windowIsDragMoved = false;

  window->ungrabNotify();
}

void WallScreen::checkAmount(int dx, int dy, int &amountX, int &amountY) {
  CompPoint point;
  CompSize size;
...
bool WallScreen::initiate(CompAction *action, CompAction::State state,
                          CompOption::Vector &options, Direction dir,
                          bool withWin) {
...
bool WallScreen::terminate(CompAction *action, CompAction::State state,
...
bool WallScreen::initiateFlip(Direction direction, CompAction::State state) {
  ...
  const bool allowFlipDnd =
      (state & CompAction::StateInitEdgeDnd) && optionGetEdgeflipDnd();
  const bool allowFlipMove = (windowIsDragMoved && optionGetEdgeflipMove());
  const bool allowFlipPointer = optionGetEdgeflipPointer();

  if (!allowFlipDnd && !allowFlipMove && !allowFlipPointer)
    return false;

  /* Check if edge flipping should be allowed based on workspace mode */
  if (screen->outputDevs().size() > 1) {
    if (optionGetMmmode() == MmmodePrimaryMonitorOnly) {
      int primaryOutputId = getPrimaryOutputDevice();
      int currentOutput = screen->outputDeviceForPoint(mouse.x(), mouse.y());

      if (currentOutput != primaryOutputId) {
        /* Don't allow edge flipping if not on primary monitor */
        return false;
      }
    }
    /* For per-monitor mode, edge flipping is allowed on each monitor separately
     */
  }
...
inline void wallDrawQuad(const GLMatrix &transform, GLTexture::Matrix *matrix,
                         BOX *box) {
...
void WallScreen::drawCairoTextureOnScreen(const GLMatrix &transform) {
...
void WallScreen::paint(CompOutput::ptrList &outputs, unsigned int mask) {
  if (moving && outputs.size() > 1) {
    if (optionGetMmmode() == MmmodeSwitchAll) {
      outputs.clear();
      outputs.push_back(&screen->fullscreenOutput());
    } else if (optionGetMmmode() == MmmodePrimaryMonitorOnly) {
      /* Only show animation on primary monitor */
      int primaryOutputId = getPrimaryOutputDevice();
      CompOutput::ptrList::iterator it = outputs.begin();

      while (it != outputs.end()) {
        if ((*it)->id() != (unsigned int)primaryOutputId)
          it = outputs.erase(it);
        else
          ++it;
      }
    }
    /* For MmmodeSwitchSeparately and MmmodePerMonitorWorkspaces,
     * keep outputs as-is to show animation on each monitor separately */
  }

  cScreen->paint(outputs, mask);
}

bool WallScreen::glPaintOutput(const GLScreenPaintAttrib &attrib,
                               const GLMatrix &matrix, const CompRegion &region,
  ...
  if (optionGetShowSwitcher() && (moving || showPreview || boxTimeout) &&
    ...
    if (optionGetMiniscreen()) {
      ...
      for (unsigned int j = 0; j < (unsigned int)screen->vpSize().height();
           j++) {
        for (unsigned int i = 0; i < (unsigned int)screen->vpSize().width();
             i++) {
          float mx, my;
          unsigned int msMask;
          CompPoint vp(i, j);

          mx = firstViewportX + (i * (viewportWidth + viewportBorder));
          my = firstViewportY + (j * (viewportHeight + viewportBorder));
          ...
          if (vp == screen->vp() && (moving || boxTimeout || showPreview)) {
            mSAttribs.brightness = BRIGHT;
          }

          cScreen->setWindowPaintOffset(
              (screen->vp().x() - i) * screen->width(),
              (screen->vp().y() - j) * screen->height());

          msMask = mask | PAINT_SCREEN_TRANSFORMED_MASK;

          glScreen->glPaintTransformedOutput(attrib, matrix, region, output,
                                             msMask);
        }
      }
      transform = NoTransformation;
...
void WallScreen::preparePaint(int msSinceLastPaint) {
  ...
  if (moving) {
    ...
    if (moveWindow) {
      ...
      if (window) {
        ...
        window->moveToViewportPosition(moveWindowX - dx, moveWindowY - dy,
                                       true);
      }
    }
  }
...
void WallScreen::glPaintTransformedOutput(const GLScreenPaintAttrib &attrib,
  ...
  if (transform == MiniScreen) {
    ...
    sMatrix.translate(-(float)output->x1() / (float)output->width(),
                      (float)output->y1() / (float)output->height(), 0.0f);
    sMatrix.translate(0.0f, 0.0f, -DEFAULT_Z_CAMERA);

    sMatrix.translate(mSAttribs.xTranslate, mSAttribs.yTranslate, mSzCamera);
    ...
    sMatrix.translate((float)output->x1() / (float)output->width(),
                      -(float)output->y2() / (float)output->height(), 0.0f);

    glScreen->glPaintTransformedOutput(attrib, sMatrix, screen->region(),
                                       output, mask);
    return;
  }

  if (!moving)
    glScreen->glPaintTransformedOutput(attrib, matrix, region, output, mask);
  ...
  if (moving) {
    ...
    if (movingY) {
      ...
      if (movingX) {
        ...
        cScreen->setWindowPaintOffset((point.x() - ceil(px)) * screen->width(),
                                      (point.y() - ceil(py)) *
                                          screen->height());
      ...
      cScreen->setWindowPaintOffset((point.x() - floor(px)) * screen->width(),
                                    (point.y() - ceil(py)) * screen->height());

      sMatrix.translate(xTranslate, 0.0f, 0.0f);

      glScreen->glPaintTransformedOutput(attrib, sMatrix, outputRegion, output,
                                         mask);
    ...
    if (movingX) {
      ...
      cScreen->setWindowPaintOffset((point.x() - ceil(px)) * screen->width(),
                                    (point.y() - floor(py)) * screen->height());
...
bool WallWindow::glPaint(const GLWindowPaintAttrib &attrib,
                         const GLMatrix &matrix, const CompRegion &region,
                         unsigned int mask) {
  bool status;
  ...
  if (ws->transform == MiniScreen) {
    ...
    pA.brightness =
        attrib.brightness * ((float)ws->mSAttribs.brightness / BRIGHT);
    pA.saturation =
        attrib.saturation * ((float)ws->mSAttribs.saturation / COLOR);
    ...
    status = glWindow->glPaint(attrib, wMatrix, region,
...
void WallScreen::createCairoContexts(bool initial) {
  int width, height;

  viewportWidth =
      VIEWPORT_SWITCHER_SIZE * (float)optionGetPreviewScale() / 100.0f;
  viewportHeight =
      viewportWidth * (float)screen->height() / (float)screen->width();
...
void WallScreen::positionUpdate(const CompPoint &pos) { mouse = pos; }

int WallScreen::getPrimaryOutputDevice() {
  /* Try to find the primary output device.
   * Primary monitor is usually the one containing (0,0) or has the name
   * "primary"
   */
  CompOutput::vector &outputs = screen->outputDevs();

  /* First try to find output with name containing "primary" */
  foreach (CompOutput &output, outputs) {
    CompString name = output.name();
    if (name.find("primary") != CompString::npos ||
        name.find("Primary") != CompString::npos) {
      return output.id();
    }
  }

  /* Fallback: find output containing point (0,0) */
  return screen->outputDeviceForPoint(0, 0);
}

void WallScreen::toggleEdges(bool enabled) {
  WALL_SCREEN(screen);
...
void WallScreen::optionChanged(CompOption *opt, WallOptions::Options num) {
  switch (num) {
...
bool WallScreen::setOptionForPlugin(const char *plugin, const char *name,
...
void WallScreen::matchExpHandlerChanged() {
...
void WallScreen::matchPropertyChanged(CompWindow *window) {
  WALL_WINDOW(window);
...
WallScreen::WallScreen(CompScreen *screen)
    : PluginClassHandler<WallScreen, CompScreen>(screen), WallOptions(),
      cScreen(CompositeScreen::get(screen)), glScreen(GLScreen::get(screen)),
      moving(false), showPreview(false), direction(-1), boxTimeout(0),
      grabIndex(0), timer(0), moveWindow(None), focusDefault(true),
      transform(NoTransformation), windowIsDragMoved(false) {
  ...
  /* Initialize mouse polling */
  pollHandle.setCallback(boost::bind(&WallScreen::positionUpdate, this, _1));
  pollHandle.start();

  createCairoContexts(true);

#define setAction(action, dir, win)                                            \
  optionSet##action##Initiate(                                                 \
      boost::bind(&WallScreen::initiate, this, _1, _2, _3, dir, win));         \
  optionSet##action##Terminate(                                                \
      boost::bind(&WallScreen::terminate, this, _1, _2, _3))

#define setFlipAction(action, dir)                                             \
  optionSet##action##Initiate(                                                 \
      boost::bind(&WallScreen::initiateFlip, this, dir, _2))
...
WallWindow::WallWindow(CompWindow *window)
    : PluginClassHandler<WallWindow, CompWindow>(window), window(window),
