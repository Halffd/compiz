...
/* enums */
typedef enum { NoTransformation, MiniScreen, Sliding } ScreenTransformation;
...
/* classes */
class WallScreen : public ScreenInterface,
                   public CompositeScreenInterface,
                   public GLScreenInterface,
                   public PluginClassHandler<WallScreen, CompScreen>,
                   public WallOptions {
public:
  enum Direction { Up = 0, Left, Down, Right, Next, Prev };
  ...
  void glPaintTransformedOutput(const GLScreenPaintAttrib &, const GLMatrix &,
                                const CompRegion &, CompOutput *, unsigned int);
...
class WallWindow : public WindowInterface,
...
#define WALL_SCREEN(s) WallScreen *ws = WallScreen::get(s)

#define WALL_WINDOW(w) WallWindow *ww = WallWindow::get(w)
